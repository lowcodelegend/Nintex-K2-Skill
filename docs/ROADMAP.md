...
### 3. K2 SmartForms builder — v0.8 common-framework lifecycle rules implemented

The tool creates checked-in capture/list/content/editable-list views and multi-view forms from declarative manifests using supported K2 generation APIs. It sets and verifies a visible title for every solution view instance, consumes a selected environment header and optional footer, keeps them first/last, calls the header initialization rule with templated parameters, transfers configured values directly to header controls on Form server load, and then invokes configured View server rules. It also converts declared capture fields into SmartObject-backed dropdowns, creates named tabs and a native K2 Worklist, separates ordinary artifacts from `Admin` CRUD UX, and verifies modern Style Profiles. The corporate fixture uses the discovered PSF Nintex framework contract.
