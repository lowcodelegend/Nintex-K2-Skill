...
On this server, `PSF.FrameworkHeader` and `PSF.FrameworkFooter` form a deliberate pair. The footer's JavaScript depends on it remaining the last rendered view. With Style Profile `PSF UX v1`, the header view-instance title must be exactly `Header` for its title behavior. The form's `ServerPreRender` rule must first use `ServerDataTransfer` to write title/subtitle literals to the header instance controls `Main Header Data Label` (`17e8b422-4cd9-4657-8c56-edc2a3512977`) and `Sub Header Data Label` (`c5176a1c-8b6e-4a40-849a-9e86ef5beabc`), and must then execute the header's user `ServerPreRender` rule (`367889d2-7d9f-4828-8154-83eec5ba173b`). Passing those strings through `HeaderText`/`SubheaderText` view parameters is not the selected contract. This is environment-specific learned configuration, so discovery proposes it only when the artifacts exist and the user accepts it.
