...
- A compact four-column capture layout means two label/control pairs, not four inputs. Repacking the generated body grid to 18/32/18/32 works for short related values; Memo rows need `ColumnSpan=3` so they retain the full remaining width.
...
- The disposable `TST.Expense Entry` regression proof on live K2 deployed a four-column/read-only master View, hidden `tblDebug` variables, a cascading dropdown, editable detail, and a modern Form with the PSF framework. Definition verification proved the Form-scoped create/update item-state batches, returned-key transfer, filtered List, bypass-button suppression, and all new control metadata. The test artifacts were removed; authenticated interactive row entry remains a browser test because direct Runtime requests receive the Windows STS challenge.
