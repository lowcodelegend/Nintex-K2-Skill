...
- A K2 Style Profile is independent of the form theme and modern/legacy mode. It is associated through the form control's `StyleProfile` property: `DisplayValue`/`NameValue` contain the profile system name and `Value` contains its GUID. `FormsManager.GetStyleProfiles()` provides display name, system name, category, flags, and version without database access.
