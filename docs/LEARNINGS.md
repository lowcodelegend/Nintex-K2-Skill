...
- Category cleanup must follow artifact cleanup and run bottom-up against exact manifest-derived paths. Specialists may remove only their empty fixed descendants; the final builder checkpoint may remove the shared root. A category with any child or artifact link is retained and reported, missing categories are idempotent success, and pre-existing Style Profile categories remain externally owned.
...
- Modern K2 colour schemes are a variable-contract problem, not a button-selector problem. On the current 5.10 server, `_Dynamic/Variables_Dynamic.css` exposes 410 colour-bearing custom properties and 31,129 contextual redeclarations across nested Forms, dialogs, toolbars, selected rows, controls, icons, Worklist, progress, and charts. Generate from the target installation, validate actual-to-expected coverage before and after minification, load that adapter before selector polish, and retain authenticated computed-colour screenshots/reports. Counts must be rediscovered after each K2 upgrade.
...
- K2 Designer hydration applies the metadata `InitialValue` contract for Form-level Label/DataLabel Boolean properties more strictly than Runtime or `SourceCode.Forms.Authoring.Form.FromXml/ToXml`. A generated Label/DataLabel that omits `LiteralVal` produces one `String was not recognized as a valid Boolean` popup per control. Persist `LiteralVal=false` with complete `DisplayValue`, `NameValue`, and `Value` elements, and make verification require every metadata-initialized Boolean property. `Width=100%` and native action-cell alignment Styles were independently proven safe.
- SmartForms cleanup ownership follows View ownership: validations declared for `reuseExisting=true` Views remain verification dependencies, but their shared native validation patterns must never be deleted by cleanup.
...
- Master-detail Create/Update paths must be one native `IfLogicalHandler` plus its immediate `Type="Else"` / `ElseLogicalHandler`, not sibling blank/not-blank conditions. K2 resumes synchronous rules after a modal closes; because Create can populate the key, a later sibling not-blank condition may then run Update during the same click.
- A View-owned master Create seam must synchronously re-read the new row after receiving its key. Literal Create defaults can persist correctly while hidden View fields remain null; the Read hydrates those fields before child persistence and preserves them for a later intentional Update. Live verification should also round-trip every generated definition through the installed `SourceCode.Forms.Authoring` model.
...
- The installed native `Progress` control advertises `UnavailableOnFormLevel`. K2 accepts and runs a Form definition containing it, and `SourceCode.Forms.Authoring.Form.FromXml`/`ToXml` also round-trips it, but the browser Designer can fail while hydrating the Form. Guided Forms therefore use a native Data Label for `Step N of M: <label>` progress text and keep the stable `prgJourneyStepN` names consumed by the Northstar Style Profile; View-level lifecycle trackers continue to use `Progress`. Form verification must compare direct Form control types with installed availability flags in addition to the authoring-model round trip.
...
# 2026-07-27: Boolean control properties need all three Designer representations

- K2's `SourceCode.Forms.Authoring.Form.FromXml`/`ToXml` round trip accepts a Boolean control property whose `DisplayValue` and `Value` are valid but whose `NameValue` is empty.
- The browser Designer property editor does not: opening the Form can throw `String was not recognized as a valid Boolean`.
- Emit matching `DisplayValue`, `NameValue`, and `Value` for every explicitly stored Boolean control property. Verification must derive Boolean property names from the installed control metadata and validate the complete triple.
- For a hidden master-detail review tab, `Panel.IsVisible` must therefore be stored as `false` in all three representations. Installing a corrected binary does not rewrite an existing Form; repair requires Form regeneration (normally `--forms-only` when its Views are already known-good).
