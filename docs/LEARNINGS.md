...
- A CLI save using `SaveKprx(close=true)` can still leave a designer lock tied to the CLI client session. Call `Processdataservice.UnlockProcess` with the K2-qualified identity (`K2:DOMAIN\user`) and confirm `GetUserProcessKprx.LockedBy` is empty; an unqualified Windows name can leave the browser in a two-attempt unlock loop.
...
The disposable workflow proof published and removed both designer/runtime layers. The corporate workflow JSON process `1041` has five nodes/four uniquely routed links; the SmartForms-integrated iteration published as runtime version 5, verified all three event components, confirmed Start/Task form rules, preserved the Human example's rules, and reported no designer lock for `K2:TRIALS\Administrator`.
