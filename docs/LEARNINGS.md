...
- The standard email field is Originator email (`ProcessOriginatorEmail`), with the From address supplied by an environment-field external reference. For test/demo routing, User Tasks use Originator FQN (`ProcessOriginatorFQN`) even when the manifest retains another requested assignee as production intent; every handoff records that override as errata.
- User Task actions need matching parent outcomes and stable action references. Native SmartForms tasks pass `SerialNo` and `_state`; the current test/demo baseline assigns the dynamic K2 `ProcessOriginatorFQN` field rather than a fixed identity or requested production participant.
