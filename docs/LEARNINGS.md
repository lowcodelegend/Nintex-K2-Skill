...
The disposable workflow proof published and removed both designer/runtime layers. `CW Request Approval` was deleted and recreated as JSON process `1048`, runtime process `3325` version 5, with six nodes/five unique absolute routes and native Start/Task rules. A clean headless browser profile opened it on the first attempt, all five connectors were visible without selecting a step, and the final lock record was empty. The untouched `Human example` remains the behavioral reference.
