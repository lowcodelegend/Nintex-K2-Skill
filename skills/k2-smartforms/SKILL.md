---
name: k2-smartforms
description: Generate, deploy, inspect, verify, replace, and clean up K2 Five SmartForms views and forms over existing SmartObjects using declarative JSON manifests and the bundled k2forms .NET CLI. Use for tabbed CRUD UX, native K2 Worklist/My Tasks tabs, SmartObject-backed dropdowns, administrative pages/categories, generated rules, modern themes, or repeatable SmartForms construction. Do not use for creating SmartObjects, workflows, arbitrary custom controls, general hand-authored XML definitions, or cloud Nintex Forms.
---
...
- Use modern theme mode for new forms. Leave `useLegacyTheme` omitted or set it to `false`; opt into `true` only for an application that depends on legacy-theme behavior. The CLI writes and verifies K2's `UseLegacyTheme` property explicitly because a theme name alone does not determine the rendering mode. Treat `application.theme`, `useLegacyTheme`, and `application.styleProfile` as independent settings.
