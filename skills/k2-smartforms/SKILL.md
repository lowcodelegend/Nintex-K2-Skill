...
- Use the environment common header by default. The CLI places it before solution views (and on the first tab of tabbed forms), preserves the view's inherited K2 rules, applies configured `Init` parameter values, and verifies its view GUID, title, placement, initialization action, and bindings. Use `application.commonHeader` only for an explicit environment name, exact per-solution override, or reasoned opt-out.
