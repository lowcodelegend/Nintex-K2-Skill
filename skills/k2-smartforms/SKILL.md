...
- For master-detail, use a capture master plus editable `capture-list` details and declare `form.masterDetail`. One Form Save handles master Create/Update, returned-key transfer, and child `Added`/`Changed`/`Removed` states. Suppress unfiltered detail List and bypass persistence controls; reload children only after a nonblank master key is read.
...
- Generate modern Forms with the selected Style Profile and `useLegacyTheme=false`; `theme` is legacy generator fallback only.
