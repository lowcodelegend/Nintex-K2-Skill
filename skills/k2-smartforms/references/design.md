...
Treat a shared header as an environment contract, not a copied solution artifact. Initial `k2env` discovery inventories likely headers; the agent then asks whether one should be the default, uses a user hint to inspect the exact view, and reviews its view parameters, user/system events, and calls/mappings used by existing forms. Persist the agreed view GUID, callable user initialization rule, server-side user rules, title, and parameter templates outside projects.

Unless a form has a concrete exception, `k2forms` reads the selected default environment and adds that existing view to every generated form. It places the header before solution content and on the first tab when the form is tabbed. An inherited view-rule definition is not an invocation: server-side rules on a View do not fire automatically when that View is placed on a Form. The CLI therefore creates a form `Init` lifecycle rule that explicitly calls the configured header user initialization rule and supplies its literal/template parameter values. For every configured `serverRules` entry, it also creates a form `When the server loads the Form` (`ServerPreRender`) rule that explicitly executes the matching header user rule with `DesignTemplate=ServerRuleExecute`. Verification checks both the inherited rule definition and the explicit form-level call by event definition ID. Arbitrary additional external form rules remain outside the generated contract and must be reported as errata.
