...
This follows K2's parent/child pattern: users stage lines with the editable-list Add/Edit/Delete controls, then persist the whole transaction through one Form-level Save button. K2 inheritance is tree-scoped: an automatically inherited View event carries reference metadata on its Event, Handler, Condition, and Action tree. A method action inserted under a new local Form Event/Handler is instead a local action: assign it a new `DefinitionID`, bind it to the embedded View `InstanceID`, retain complete instance-qualified input/result mappings, and omit `IsReference`/`IsInherited`. Marking only the leaf action inherited creates a partial reference tree that can pass the analyzer yet make the rule editor spin indefinitely. Every Control or ViewField mapping copied into a Form rule must carry its source/target View instance; rewrite hidden control sources to their retained ViewField before K2 strips the unplaced control. A `List` action with `ControlID` is a dropdown-population rule, not a detail load: preserve it, exclude it from detail reconciliation, and never clone it into the Save rule. Only control-free `List` actions participate in master-detail loading. A new/blank master must show an empty child list without invoking List; viewing an existing master must invoke exactly the filtered Form-level List path. Hide every generated master Item View button and the detail Save/Refresh controls so a partial or unfiltered View operation is not presented as a valid transaction; retain detail Add/Edit/Delete. End both successful Create and Update branches with a small informational popup after persistence. Never rely on a View rule to coordinate another View. Test creation with two rows, confirm the generated parent key was transferred before child persistence, create a second master, reload each and prove row isolation, then test one added, one changed, and one removed row.
