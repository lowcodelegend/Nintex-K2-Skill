...
1. The CLI wraps the generated master Create and Update method actions in dedicated View-owned custom rules named `K2Skills.MasterDetail.Create.<key>` and `K2Skills.MasterDetail.Update.<key>`. The Form invokes those `SourceType="Rule"` events rather than cloning method actions or targeting generated Create/Update button events. Create returns the generated key to the master View field. Custom View Events keep their View-name location, while their Handlers must use canonical lowercase `view`; K2's browser Rule Designer does not hydrate a View-name Handler location. The synthetic Form Save Button carries both K2's system `OnClick` declaration and its user rule; omitting the system declaration leaves a server-valid rule that the browser cannot edit. Create/Update branches use local `AdvancedCondition` nodes with the View instance carried only by each source Item, matching native Designer-authored blank/not-blank tests.
