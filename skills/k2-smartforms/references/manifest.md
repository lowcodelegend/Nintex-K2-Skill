...
Treat authentication as three separate planes:

1. **Runtime user authentication** controls how people reach Designer/Runtime, such as OIDC, forms login, Windows authentication, or anonymous Form access.
2. **K2 management authentication** controls `FormsManager` and other K2 client connections. `k2` uses the validated environment-wide Windows-integrated connection by default. A `K2SQL` security label and user such as `K2Admin` are K2 identities, not SQL Server credentials. Set `integrated` false plus `securityLabel`, `domain` when applicable, `userName`, and `passwordEnvironmentVariable` only for a proven environment-level management exception; never store the password itself.
3. **SQL Server authentication** belongs to the SQL SmartObject/service-instance layer. It is normally Windows integrated on self-hosted K2 servers. Use `sa` or another explicit SQL login only when the target SQL environment requires it. Never infer SQL authentication from OIDC or a K2 security label, and never create a solution-prefixed SQL/K2 password merely because the manifest has a solution code.

Prefer `validated environment profile → explicit approved environment exception`. Do not copy solution-specific credential names between manifests. If a manifest forces an absent variable such as `CPR_K2_PASSWORD` while the environment's integrated K2 management connection validates, remove the stale override and inherit the environment connection. Keep Runtime OIDC testing on the attended browser path; changing browser authentication never requires changing SQL Server authentication.
...
On a Form with `guidedJourney`, generation and verification place the helper at the bottom of the first physical journey screen. On a non-guided Form it remains at the bottom of the last visible panel. Its containing K2 Table Cell receives native left alignment implicitly; there is no manifest alignment field.
...
The CLI adds the journey title/description, one native read-only Data Label containing `Step N of M: <label>`, the screen title, and the screen description to every tab. This keeps the Form understandable if a selected Style Profile fails open. K2's native `Progress` control remains the View-level lifecycle control; installed metadata marks it `UnavailableOnFormLevel`, so guided Forms must not contain it. A generated Back button performs only a native Form `Focus` action. A generated Continue button first runs `ValidationGroupForEvent` when it exists, with invisible, disabled, and read-only controls ignored, then focuses the next tab. Save and the selected final action retain their owned rules rather than receiving duplicate Continue actions. A review tab hidden until Save stores the Boolean Panel `IsVisible` property with matching `DisplayValue`, `NameValue`, and `Value`; never omit or empty one representation because the server authoring round trip accepts it while the browser Designer Boolean editor does not. Native Table Cell alignment is inferred from role: Back is left; Continue, Save, workflow-free Finish, and workflow Submit are right. This is deliberately implicit because these generated actions have fixed semantics. Verification checks installed Form-control availability and Boolean-property metadata plus the exact stored `<Styles><Style IsDefault="True"><Text><Align>…` contract in addition to title/description, progress text, placement, K2 system/user event hydration, validation-before-focus ordering, Save placement, and final-action placement.
