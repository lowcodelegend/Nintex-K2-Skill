...
The master must be a `capture` View containing its key and selected Create/Update/Read methods. Each detail must be `capture-list` with `editable` and selected Create/Update/Delete/List methods. Put every required child collection in `details`; generation or integration drift never authorizes collapsing it into the master. The CLI adds one Form-level button (`saveButtonText`, default `Save`) with Create and Update branches based on whether the master key is blank. `successMessageTitle` and `successMessageBody` customize the small informational popup that executes last after either successful persistence branch; their defaults are `Saved` and `The record and its line items were saved successfully.` Create maps the returned SmartObject identity to the master View field before child foreign-key use; Update processes `Changed`, `Added`, and `Removed` states. Each method call inserted under the local Form Save Event/Handler is a local action with a new `DefinitionID`, the embedded View `InstanceID`, and complete instance-qualified mappings; it must not carry leaf-only `IsReference`/`IsInherited` flags. Full automatically inherited View event trees remain inherited and are preserved separately. The CLI removes only unfiltered, control-free detail List actions. Every `List` action with `ControlID` is lookup population and is preserved and excluded from detail counting/reconciliation, including inherited copies in workflow-created Form states. After each master Read path, a separate Form handler tests that the master key is not blank and passes it to every detail foreign-key List input. The CLI hides every generated master Item View button plus detail Save/Refresh buttons, while retaining detail Add/Edit/Delete controls for item-state editing. Verification rejects partial inheritance, missing instance identity/mappings, missing or duplicate persistence actions, unfiltered/ungated/misordered detail List paths, visible bypass buttons, missing success feedback or returned-key mappings, and persistence outside the Form Save event.
