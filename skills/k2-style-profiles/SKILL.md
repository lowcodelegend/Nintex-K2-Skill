...
8. Test authenticated Runtime Forms at desktop and mobile widths, with normal and reduced-motion browser preferences. On a page tall enough to scroll, open native K2 dropdowns above and below the fold and prove the page retains its scroll position. Also open the Form and Style Profile designers and confirm that custom runtime UI, overlays, loaders, and DOM manipulation do not execute there. For shell work, configure and run `scripts/test-runtime.ps1`; do not accept an unmeasured cold load, warm transition, timeout, or Designer boundary.
...
- Scope reduced-motion and other motion overrides only to explicitly owned Style Profile elements that declare that motion. Never apply `transition`, `transition-duration`, `animation`, `animation-duration`, `animation-iteration-count`, or `scroll-behavior` through `*`, `html:not(.designer) *`, or another universal descendant selector: changing native K2 transition timing can focus a dropdown option before its popup is positioned and jump a scrolled Runtime page to the top. `k2style doctor` rejects this pattern.
