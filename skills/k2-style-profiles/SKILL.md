...
- For the Northstar case homepage, style and arrange native semantic Views; do not replace them with injected authoritative buttons, charts, tables, or a full-page control. Keep the exact `Application navigation` and `Command palette` View titles, server-filter the palette projection, and prove authenticated visual parity against the gold-standard prototype at every required viewport. Never reparent the live command-palette View: retain its K2 ownership tree and position its marked row through the Style Profile.
