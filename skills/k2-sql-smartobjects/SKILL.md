...
- Declare every database rule that applies to user-editable form input in root `formConstraints`, including the exact SmartObject/property and SQL schema/table/column. Carry text lengths, required input, minimum length, numeric bounds, formats/patterns, and must-be-true bits explicitly; list named SQL checks in `sourceConstraints`. This is the machine-readable handoff to `$k2-smartforms` and `$k2-builder`, not a replacement for the SQL constraint itself.
...
- For user-scoped projections such as navigation search or a command palette, accept an `nvarchar` user parameter in the SQL procedure and declare `k2.smartObjects.systemValueMappings` with `value: ConnectedUserFQN`. The CLI replaces the generated method input with K2's server-side system mapping and verifies that callers cannot supply or spoof that identity.
