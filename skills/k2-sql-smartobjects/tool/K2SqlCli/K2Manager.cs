...
namespace K2SqlCli
{
    internal sealed class K2Manager
    {
        ...
        public void GenerateSmartObjects(Guid serviceInstanceGuid)
        {
            ...
            ApplyPropertyTypeOverrides(serviceInstanceGuid);
            ApplySystemValueMappings(serviceInstanceGuid);
            PlaceGeneratedSmartObjects(serviceInstanceGuid);
        }

        private void ApplyPropertyTypeOverrides(Guid serviceInstanceGuid)
        {
            if (_manifest.K2.SmartObjects.PropertyTypeOverrides.Count == 0) return;
            var generated = GetGeneratedSmartObjects(serviceInstanceGuid);
            WithSmartObjectManagementServer(delegate(SmartObjectManagementServer server)
            {
                foreach (var item in _manifest.K2.SmartObjects.PropertyTypeOverrides)
                {
                    var state = generated.SingleOrDefault(x => string.Equals(x.SystemName, item.SmartObject, StringComparison.OrdinalIgnoreCase));
                    if (state == null)
                        throw new CliException("SmartObject property type override target is not generated by this Service Instance: " + item.SmartObject);
                    var definition = XDocument.Parse(server.GetSmartObjectDefinition(state.Guid), LoadOptions.PreserveWhitespace);
                    var root = definition.Root;
                    var properties = root == null ? null : root.Elements().FirstOrDefault(x => x.Name.LocalName == "properties");
                    var property = properties == null ? null : properties.Elements().SingleOrDefault(x =>
                        x.Name.LocalName == "property" &&
                        string.Equals((string)x.Attribute("name"), item.Property, StringComparison.OrdinalIgnoreCase));
                    if (property == null)
                        throw new CliException("SmartObject property type override target does not exist: " + item.SmartObject + "." + item.Property);

                    VerifyFileOverrideSource(definition, item);
                    if (string.Equals((string)property.Attribute("type"), "file", StringComparison.OrdinalIgnoreCase))
                    {
                        Console.WriteLine("SmartObject property type: already File (" + item.SmartObject + "." + item.Property + ")");
                        continue;
                    }
                    property.SetAttributeValue("type", "file");
                    server.PublishSmartObject(definition.ToString(SaveOptions.DisableFormatting));
                    Console.WriteLine("SmartObject property type: File (" + item.SmartObject + "." + item.Property + ")");
                }
                return 0;
            });
        }

        private static void VerifyFileOverrideSource(XDocument definition, SmartObjectPropertyTypeOverride item)
        {
            var serviceProperties = definition.Descendants().Where(x =>
                x.Name.LocalName == "property" &&
                string.Equals((string)x.Attribute("name"), item.Property, StringComparison.OrdinalIgnoreCase) &&
                x.Attribute("sotype") != null).ToList();
            if (serviceProperties.Count == 0)
                throw new CliException("SmartObject File override cannot find the SQL service property mapping for " + item.SmartObject + "." + item.Property + ".");
            foreach (var property in serviceProperties)
            {
                var sqlType = property.Descendants().Where(x => x.Name.LocalName == "key" &&
                    string.Equals((string)x.Attribute("name"), "DataType", StringComparison.OrdinalIgnoreCase))
                    .Select(x => x.Value).FirstOrDefault();
                var maximum = property.Descendants().Where(x => x.Name.LocalName == "key" &&
                    string.Equals((string)x.Attribute("name"), "CharMaxLength", StringComparison.OrdinalIgnoreCase))
                    .Select(x => x.Value).FirstOrDefault();
                if (!string.Equals((string)property.Attribute("type"), "System.String", StringComparison.OrdinalIgnoreCase) ||
                    !string.Equals(sqlType, "varchar", StringComparison.OrdinalIgnoreCase) ||
                    (!string.Equals(maximum, "-1", StringComparison.Ordinal) &&
                     !string.Equals(maximum, int.MaxValue.ToString(CultureInfo.InvariantCulture), StringComparison.Ordinal)))
                    throw new CliException("SmartObject File override requires a SQL varchar(max) service property, but " +
                        item.SmartObject + "." + item.Property + " maps as " + sqlType + "(" + maximum + ").");
            }
        }

        private void ApplySystemValueMappings(Guid serviceInstanceGuid)
        {
            if (_manifest.K2.SmartObjects.SystemValueMappings.Count == 0) return;
            var generated = GetGeneratedSmartObjects(serviceInstanceGuid);
            WithSmartObjectManagementServer(delegate(SmartObjectManagementServer server)
            {
                foreach (var item in _manifest.K2.SmartObjects.SystemValueMappings)
                {
                    var state = generated.SingleOrDefault(x =>
                        string.Equals(x.SystemName, item.SmartObject, StringComparison.OrdinalIgnoreCase));
                    if (state == null)
                        throw new CliException("SmartObject system-value mapping target is not generated by this Service Instance: " + item.SmartObject);
                    var definition = server.GetSmartObjectDefinition(state.Guid);
                    var updated = ApplySystemValueMapping(definition, item);
                    if (string.Equals(updated, definition, StringComparison.Ordinal))
                    {
                        Console.WriteLine("SmartObject system value: already mapped (" +
                            item.SmartObject + "." + item.Method + "." + item.Target + "=" + item.Value + ")");
                        continue;
                    }
                    server.PublishSmartObject(updated);
                    Console.WriteLine("SmartObject system value: mapped (" +
                        item.SmartObject + "." + item.Method + "." + item.Target + "=" + item.Value + ")");
                }
                return 0;
            });
        ...
        internal static string ApplySystemValueMapping(string xml, SmartObjectSystemValueMapping item)
        {
            var document = XDocument.Parse(xml, LoadOptions.PreserveWhitespace);
            var root = document.Root;
            var rootMethods = root == null ? null : root.Elements().FirstOrDefault(x => x.Name.LocalName == "methods");
            var method = rootMethods == null ? null : rootMethods.Elements().SingleOrDefault(x =>
                x.Name.LocalName == "method" &&
                string.Equals((string)x.Attribute("name"), item.Method, StringComparison.OrdinalIgnoreCase));
            if (method == null)
                throw new CliException("SmartObject system-value mapping method does not exist: " +
                    item.SmartObject + "." + item.Method);

            var serviceMethods = method.Elements().Where(x => x.Name.LocalName == "serviceinstances")
                .SelectMany(x => x.Descendants().Where(candidate =>
                    candidate.Name.LocalName == "method" &&
                    string.Equals((string)candidate.Attribute("name"), item.Method, StringComparison.OrdinalIgnoreCase)))
                .ToList();
            var targets = serviceMethods.SelectMany(serviceMethod =>
                    serviceMethod.Elements().Where(x => x.Name.LocalName == "parameters")
                        .SelectMany(x => x.Elements().Where(parameter =>
                            parameter.Name.LocalName == "parameter" &&
                            string.Equals((string)parameter.Attribute("name"), item.Target, StringComparison.OrdinalIgnoreCase)))
                    .Concat(serviceMethod.Elements().Where(x => x.Name.LocalName == "properties")
                        .SelectMany(x => x.Elements().Where(property =>
                            property.Name.LocalName == "property" &&
                            string.Equals((string)property.Attribute("name"), item.Target, StringComparison.OrdinalIgnoreCase)))))
                .ToList();
            if (targets.Count != 1)
                throw new CliException("SmartObject system-value mapping must resolve exactly one service target, found " +
                    targets.Count + ": " + item.SmartObject + "." + item.Method + "." + item.Target);
            var target = targets[0];
            var targetType = (string)target.Attribute("type") ?? string.Empty;
            var targetSmartType = (string)target.Attribute("sotype") ?? string.Empty;
            if (targetType.IndexOf("String", StringComparison.OrdinalIgnoreCase) < 0 &&
                !string.Equals(targetSmartType, "text", StringComparison.OrdinalIgnoreCase) &&
                !string.Equals(targetSmartType, "memo", StringComparison.OrdinalIgnoreCase))
                throw new CliException("ConnectedUserFQN requires a Text target: " +
                    item.SmartObject + "." + item.Method + "." + item.Target);

            var mappings = target.Elements().FirstOrDefault(x => x.Name.LocalName == "mappings");
            if (mappings == null)
            {
                mappings = new XElement(target.Name.Namespace + "mappings");
                target.Add(mappings);
            }
            var alreadyMapped = mappings.Elements().Count(x =>
                x.Name.LocalName == "mapping" &&
                string.Equals((string)x.Attribute("type"), "system", StringComparison.OrdinalIgnoreCase) &&
                string.Equals(x.Elements().FirstOrDefault(value => value.Name.LocalName == "value") == null
                    ? null
                    : x.Elements().First(value => value.Name.LocalName == "value").Value,
                    item.Value, StringComparison.OrdinalIgnoreCase)) == 1 &&
                mappings.Elements().Count() == 1;
            if (!alreadyMapped)
            {
                mappings.RemoveNodes();
                mappings.Add(new XElement(target.Name.Namespace + "mapping",
                    new XAttribute("type", "system"),
                    new XElement(target.Name.Namespace + "value", item.Value)));
            }

            RemoveNamedChild(method, "parameters", "parameter", item.Target);
            RemoveNamedChild(method, "input", "property", item.Target);
            var validation = method.Elements().FirstOrDefault(x => x.Name.LocalName == "validation");
            if (validation != null)
                RemoveNamedChild(validation, "requiredproperties", "property", item.Target);

            return document.ToString(SaveOptions.DisableFormatting);
        }

        private static void RemoveNamedChild(XElement parent, string containerName, string childName, string target)
        {
            var container = parent.Elements().FirstOrDefault(x => x.Name.LocalName == containerName);
            if (container == null) return;
            foreach (var child in container.Elements().Where(x =>
                x.Name.LocalName == childName &&
                string.Equals((string)x.Attribute("name"), target, StringComparison.OrdinalIgnoreCase)).ToList())
                child.Remove();
        }
        ...
        public void Verify()
        {
            ...
            VerifyPropertyTypeOverrides();
            VerifySystemValueMappings(instance.Guid);
        ...
        private void VerifyPropertyTypeOverrides()
        {
            if (_manifest.K2.SmartObjects.PropertyTypeOverrides.Count == 0) return;
            ...
            try
            {
                ...
                foreach (var item in _manifest.K2.SmartObjects.PropertyTypeOverrides)
                {
                    var smartObject = server.GetSmartObject(item.SmartObject);
                    var property = smartObject.Properties.Cast<SmartProperty>().SingleOrDefault(x =>
                        string.Equals(x.Name, item.Property, StringComparison.OrdinalIgnoreCase));
                    if (property == null || !string.Equals(property.Type.ToString(), item.Type, StringComparison.OrdinalIgnoreCase))
                        throw new CliException("SmartObject property type verification failed for " + item.SmartObject + "." +
                            item.Property + ": expected " + item.Type + ", found " + (property == null ? "<missing>" : property.Type.ToString()) + ".");
                    Console.WriteLine("SmartObject property type verification: OK (" + item.SmartObject + "." + item.Property + "=" + item.Type + ")");
                }
            }
            finally
            {
        ...
        private void VerifySystemValueMappings(Guid serviceInstanceGuid)
        {
            if (_manifest.K2.SmartObjects.SystemValueMappings.Count == 0) return;
            var generated = GetGeneratedSmartObjects(serviceInstanceGuid);
            WithSmartObjectManagementServer(delegate(SmartObjectManagementServer server)
            {
                foreach (var item in _manifest.K2.SmartObjects.SystemValueMappings)
                {
                    var state = generated.SingleOrDefault(x =>
                        string.Equals(x.SystemName, item.SmartObject, StringComparison.OrdinalIgnoreCase));
                    if (state == null)
                        throw new CliException("SmartObject system-value verification target is not generated by this Service Instance: " + item.SmartObject);
                    var definition = server.GetSmartObjectDefinition(state.Guid);
                    var normalized = ApplySystemValueMapping(definition, item);
                    if (!string.Equals(normalized, definition, StringComparison.Ordinal))
                        throw new CliException("SmartObject system-value mapping verification failed for " +
                            item.SmartObject + "." + item.Method + "." + item.Target + ".");
                    Console.WriteLine("SmartObject system-value verification: OK (" +
                        item.SmartObject + "." + item.Method + "." + item.Target + "=" + item.Value + ")");
                }
                return 0;
            });
