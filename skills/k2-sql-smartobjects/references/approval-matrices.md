...
- A missing condition makes that rule a wildcard for the dimension. A dimensioned rule should normally have a lower numeric `priority` than its fallback rule.
...
`approverType` accepts `User`, `Group`, `Role`, or `K2String`. `approver` is the destination string consumed by K2. Use fully qualified K2 identity strings where required, for example `K2:DOMAIN\\username`. The special `$designer` value is replaced during deployment with `K2:<current Windows identity>`; this provides usable test/demo rules without storing a password. Always emit the `approval-matrix-demo-identities` erratum and replace or confirm these values before production.

Generate Admin capture/list views and a maintenance form over the table SmartObject. Do not expose `RuleId` as a user-entered value, and use dropdowns for normalized dimension keys such as Department. Browser-test exact lower/upper thresholds, wildcard fallback, every stage transition, rejection, and the no-more-stages sentinel.
