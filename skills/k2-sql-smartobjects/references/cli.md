...
| `plan` | No | Report whether the database and Service Instance will be created or updated and list scripts, grants, generation flags, target Data category, and assertions. |
| `deploy --confirm` | Yes | Deploy SQL, register/update/refresh the Service Instance, generate SmartObjects, place them in the configured solution's Data category, and verify. |
| `verify` | No | Assert SQL objects/queries, generated SmartObjects and category placement, then run eligible List methods. |
...
The database is created only when absent. SQL scripts rerun on every deployment and therefore must be idempotent. The Service Instance is matched by `systemName`; an existing instance is updated and refreshed. SmartObject generation defaults to creating new and updating existing generated SmartObjects while retaining removed objects. When `application.rootCategoryPath` is set, every deployment reasserts `<root>\Data` placement after Service Instance refresh and generation.
