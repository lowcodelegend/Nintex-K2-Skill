...
$project = Join-Path $skillRoot 'tool\K2SmartBoxCli\K2SmartBoxCli.csproj'
...
$output = Join-Path $skillRoot "tool\K2SmartBoxCli\bin\$Configuration\k2smartbox.exe"
& $output selftest | Out-Host
if ($LASTEXITCODE -ne 0) {
    throw "k2smartbox self-test failed with exit code $LASTEXITCODE."
}
Write-Output $output
