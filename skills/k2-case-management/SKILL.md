...
6. Start the experience from [the canonical case UX](assets/case-ux.yaml), including its required native SmartForms Northstar homepage, extend it instead of rebuilding the shell and standard case components, and follow [the UX contract](references/ux-contract.md). Use native K2 artifacts for navigation, content, actions, rules, accessibility, and persistence; use a modern Web Component only for the bounded command-palette gap. Define optional AI assistance and its human oversight, solution-specific Forms, queues, dashboards, reports, permissions, retention, operations, and recovery.
...
Read [architecture](references/architecture.md), [data model](references/data-model.md), [workflow pattern](references/workflow-pattern.md), [business rules](references/business-rules.md), [AI assistance](references/ai-assistance.md), [UI patterns](references/ui-patterns.md), [UX contract](references/ux-contract.md), and [naming](references/naming-conventions.md). Use [the example](references/example-case-type.md) as an adaptation guide. Copy [case type](assets/case-type-definition.yaml), [stage](assets/stage-definition.yaml), and the [case UX overlay](assets/case-ux-overlay.yaml). Compose the UX overlay with `& scripts/compose-case-ux.ps1 -Overlay <case-ux.yaml> -Output <composed-case-ux.json>`, then run `& scripts/validate-case-model.ps1 -Manifest <case-type.yaml>` and `& scripts/validate-case-ux.ps1 -Manifest <composed-case-ux.json>`. Package and register [the bounded command palette](assets/northstar-command-palette) through `$k2-smartforms-web-components`, prepare the reusable `northstar-native-homepage` Style Profile through `$k2-style-profiles`, then compile mapped UX with `& scripts/compile-case-ux-smartforms.ps1 -Ux <composed.json> -Mapping <mapping.yaml> -BaseManifest <solution-smartforms.json> -Output <combined-smartforms.json>` and deploy it through `$k2-smartforms`. The compiler emits a native navigation List View, a bounded palette capture View, native KPI/chart/queue Views, and one modern Form with legacy themes and test Pre-fill explicitly disabled. The `.yaml` assets use JSON-compatible YAML; the optional Python validators also accept general YAML when PyYAML is installed.
...
- Treat the native SmartForms Northstar homepage as the default role landing for case operations. The full-page `northstar-case-homepage` control is a temporary visual oracle only and must not be generated into new solutions. Use the modern `northstar-command-palette` only for search/command interaction that native controls cannot match; never substitute a legacy DLL custom control.
