...
6. Start the experience from [the canonical case UX](assets/case-ux.yaml), including its required Northstar homepage, extend it instead of rebuilding the shell and standard case components, and follow [the UX contract](references/ux-contract.md). Define optional AI assistance and its human oversight, solution-specific Forms, queues, dashboards, reports, permissions, retention, operations, and recovery.
...
- Treat the Northstar Web Component homepage as the default role landing for case operations. A native fallback is permitted only as a documented erratum; never substitute a legacy DLL custom control.
