...
- When a Northstar Style Profile is selected, include the governed `Application navigation` List View and the one native `Command palette` View on the first screen. The reusable profile hides the navigation source after loading it, preserves the application shell on a cold direct link, keeps only the palette's marked row operational when K2 hides the first panel, converts the original K2 tab strip into the desktop/mobile stepper, merges the active native screen into one content card, and adds contextual guidance. It must not clone tabs, Views, or the palette control. Upcoming tabs remain non-clickable until a native Continue/Save/Focus path reaches them, so the tab strip cannot bypass current-screen validation. Rule behavior and persistence remain native and Designer-editable.

Browser-test populated, validation-error, long-content, empty-collection, Back/Continue, Save, Review, second intentional Save, and the selected final-action path at every declared viewport. Confirm a failed Continue stays on the current screen and focuses or marks the offending control. After each successful transition, open the real command palette, focus its search input, and return at least one governed option; do this at desktop and mobile widths. Measure each completed-step mark and require its fixed-size CSS check to remain centered in the circle. Confirm Save invokes Create or Update exactly once. In `complete` mode, confirm Finish never starts a workflow or claims submission; in `workflow` mode, confirm Submit starts the parent workflow exactly once.
