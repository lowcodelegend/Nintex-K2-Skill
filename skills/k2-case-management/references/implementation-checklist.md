...
- Verification: template validator passes; the landing page is native SmartForms with legacy themes disabled and Pre-fill disabled; navigation and palette projections match their exact ordered contracts; palette results are bounded, authorized, keyboard accessible, and route through a View-owned native event; dashboard widgets bind governed rows once from the owning View Init rule through `NewItems`; links resolve; all canonical entities are mapped or explicitly external; only explicitly requested visualization data alternatives are present; strict native-to-Northstar visual comparison passes at every declared viewport; the authenticated desktop/tablet/mobile evidence report passes for every primary Form; and ordinary authenticated browser scenarios cover creation, transition, re-entry, hold/resume, escalation, decision, failure recovery and closure. Record known K2 diagnostics and responsive control limitations as errata rather than suppressing them.
