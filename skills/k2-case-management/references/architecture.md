...
Keep the agent framework transport-neutral. Let the remote MCP server adapt its stable discovery, draft, validation, preview, and creation facade. Resolve solution-specific fields through an immutable case creation contract and dispatch only to an allowlisted case-type creation adapter; never generate dynamic SQL from an extension payload. Keep instance EvidenceItems and effective-dated CaseTypePolicyDocuments as different authorization and retrieval domains.

Host [the remote MCP adapter](agentic-case-mcp.md) as a separate internal service reachable from Langflow over HTTPS Streamable HTTP. Authenticate every request, bind the verified identity to authorization and draft ownership, allowlist Host/Origin values, and keep provider plugins server-side. The MCP server is a protocol and governance boundary, not the lifecycle system of record.
