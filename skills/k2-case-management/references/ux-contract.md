...
The Style Profile may annotate existing K2 rows, cells, and controls with narrowly prefixed classes and position the original palette row. It must not move bound KPI labels/Data Labels into replacement cards, copy native values into an injected dashboard, or append synthetic actions into chart Views. Keep the original K2 ownership tree intact; style its existing containers in place.
