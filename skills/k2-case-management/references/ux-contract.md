...
Declare `homepage.commandPalette.viewTitle` as the explicit shell/compiler contract; use `Command palette` for the canonical Northstar profile. An `agenticChat` replacement must retain that title. Runtime evidence must prove the real View row occupies the `.k2sp-command-palette-host` geometry without reparenting the K2 View, the fallback is hidden and non-interactive, and real CDP pointer input plus Ctrl/Cmd+K each open the same control exactly once.
...
Treat capture as an iterative build loop, not final documentation: capture the prototype and authenticated Runtime with the same browser build and viewport, inspect the visible mismatch regions, correct the reusable manifest/compiler/Style Profile contract, redeploy, and capture again. Do not claim parity from DOM assertions alone. The capture helper uses a dependency-free Node DevTools driver and accepts evidence only when exactly one Northstar shell exists, its styles and content are ready, and the native Form has completed enough to release its loading mask. Preserve the palette View in its original K2 ownership tree; use stable semantic classes and CSS positioning for the top-bar treatment rather than reparenting the live View. On guided Forms, keep only its marked row operational when K2 hides the first-screen panel, and prove it still opens, focuses, and returns governed results after real native navigation. Measure completed-step check geometry as well as inspecting the rendered desktop/mobile screenshots.
...
Before compiling initiation, apply [the initiation UX contract](initiation-ux.md). `initiation.guidedMode` defaults to `auto`: a composed journey with at least three coherent tasks becomes guided when it has a large field set, a collection, resumable draft behavior, or review-before-submit. The compiler emits native journey title/description controls, Form-supported `Step N of M` progress text on each screen, a distinct short step label and content-card title, current-screen validation before Continue, non-destructive Back, master-detail Save on the penultimate screen, and exactly one final action on Review. With a selected Northstar profile it also places the governed application-navigation source on the first screen and normalizes entry/review Views to responsive label-above cards; the profile then styles the original K2 tab strip as the in-shell stepper and blocks forward tab-click bypass without replacing native rules. K2's native `Progress` control is reserved for View-level lifecycle trackers because installed metadata marks it unavailable on Forms. `initiation.finalActionMode` defaults to `workflow`; use `complete` when workflows are deliberately deferred. Use `initiation.stepTabs` only when real task-specific Views can place every initiation View once; otherwise retain the portable Details → Evidence → Review physical mapping rather than manufacturing decorative screens.
