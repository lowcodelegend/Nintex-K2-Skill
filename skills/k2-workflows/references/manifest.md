...
- `call-subworkflow`: generate Start → Call Sub Workflow → End using K2's native component. `workflow.callSubWorkflow.workflow` is the exact deployed `<category>\\<workflow>` full name; `account` defaults to `Originator`; `waitFor` is `none`, `all`, or `first`. Use `all` for one synchronous child and deploy that child first. This primitive does not invent case routing or child data mappings.
- `case-lifecycle`: generate a canonical case parent with one resolve/load activity, terminal and configured-stage decisions, eight synchronous child calls, and child-to-resolver loops. `caseLifecycle.resolverSmartObject`/`resolverMethod` performs the idempotent transition and upserts lifecycle state; `stateSmartObject`/`stateMethod` must be a normal Read contract. Map the parent case field through `resolverCaseIdInput` and `stateCaseIdInput`; the legacy `caseIdInput` may still supply both when the live method inputs have the same system name. `stageInstanceProperty`, `stageWorkflowProperty`, and `isTerminalProperty` are mapped into parent fields. `stageWorkflows` lists the eight exact child workflow full names in lifecycle order, while `workflowIds` maps those same names to their deployed Designer JSON process IDs. Each child must declare `childStageInstanceInput` as a scalar process field even when it is SmartForms-integrated.
...
The designer environment is `smartforms`; it is an environment identifier from the hosted designer, not a server DNS name. For Integrated Windows authentication, set `integrated=true` and the installed Windows security label. For a non-integrated label such as `K2SQL`, set `integrated=false`, `securityLabel`, optional `domain`, `userName`, and `passwordEnvironmentVariable`. `workflowPort` defaults to `5252`. Never put a password in a workflow manifest; launch live commands through the `$k2-builder` environment-profile wrapper so its DPAPI-protected credential is exposed only to the child process.
...
Set `email.enabled=false` only when the target environment has no working K2 Email activity. For non-SmartForms direct-routing workflows this removes the standalone email nodes while preserving pending status, task assignment, the two task-action decision branches, and approved/rejected status persistence. Record the missing mail capability as environment errata; disabling email is not evidence of successful delivery.
...
For a case-lifecycle parent started from a reviewed draft, set `smartForms.startOnly=true` and `primarySmartObject` to the exact SmartObject exposed by the initiation Form. `form`, `startState`, and `startRuleContains` remain required; task configuration and Task-state integration are skipped. Use a dedicated submit-rule fragment and normally keep `makeStartStateDefault=false`.
