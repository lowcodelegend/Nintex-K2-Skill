...
- `request-approval`: generate the tested Human-example topology: SmartForm Start → pending status Update plus Originator Email → User Task → Decision → approved/rejected status Update plus Originator Email. For the current testing/demo baseline, the generated task destination is always workflow Originator (`ProcessOriginatorFQN`) even when `userTask.assignees` names a different requested production participant. SmartForms mode requires exactly two task actions. The Update method and form must already exist. The form must expose the request SmartObject as its primary Create reference. The workflow maps that item reference's identifier into all three Update events, adds workflow-specific Start/Task states, and passes `SerialNo` plus `_state` to the task form.
...
Dynamic identity/environment tokens include `$environment:<field>`, `$originator`, and `$originatorManager`. `userTask.assignees` records requested production intent, while typed generation currently overrides the effective task destination to `$originator`; `plan`, `render`, `deploy`, and `verify` report this as test/demo errata. `userTask.notification.enabled=true` selects K2's built-in task notification and requires `subject` and `body`. Those templates preserve free text and recognize `{{request.<Property>}}` for a property returned by the request SmartObject Read method, `{{task.participantName}}`, and `{{task.worklistLink}}`; set `richText=true` for HTML bodies. The worklist token emits K2's native hyperlink expression.
