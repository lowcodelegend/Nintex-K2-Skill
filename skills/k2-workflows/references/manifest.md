...
Dynamic identity/environment tokens include `$environment:<field>`, `$originator`, and `$originatorManager`. For direct routing, `userTask.assignees` records requested production intent while typed generation overrides the effective task destination to `$originator`; `plan`, `render`, `deploy`, and `verify` report this as test/demo errata. Matrix routing instead uses the resolver output and reports matrix placeholder identities upstream. `userTask.notification.enabled=true` selects K2's built-in task notification and requires `subject` and `body`. Those templates preserve free text and recognize `{{request.<Property>}}` for a property returned by the request SmartObject Read method, `{{task.participantName}}`, and `{{task.worklistLink}}`; set `richText=true` for HTML bodies. The worklist token emits K2's native hyperlink expression.
