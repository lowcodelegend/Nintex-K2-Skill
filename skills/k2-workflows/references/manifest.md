...
- `case-lifecycle`: generate a canonical case parent with one resolve/load activity, terminal and configured-stage decisions, eight synchronous child calls, and child-to-resolver loops. `caseLifecycle.resolverSmartObject`/`resolverMethod` performs the idempotent transition and upserts lifecycle state; `stateSmartObject`/`stateMethod` must be a normal Read contract keyed by `caseIdInput`. `stageInstanceProperty` and `stageWorkflowProperty` are mapped into parent fields. `workflowIds` maps every exact child workflow full name to its deployed Designer JSON process ID. Each child must declare `childStageInstanceInput` as a scalar process field even when it is SmartForms-integrated.
