...
On deployment, an existing workflow-specific Start integration whose default flag differs from the manifest is removed and recreated with the declared flag. Other states and unrelated rules are preserved, but manual edits inside that generated Start state are not merged; review or export important customizations first.
