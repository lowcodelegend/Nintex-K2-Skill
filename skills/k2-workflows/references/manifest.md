...
On deployment, an existing workflow-specific Start integration whose default flag differs from the manifest is removed and recreated before workflow publication with the declared flag. Reconciliation runs first because `SaveKprx` can check out the integrated form during publication. Other states and unrelated rules are preserved, but manual edits inside that generated Start state are not merged; review or export important customizations first.
