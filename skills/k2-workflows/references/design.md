...
Keep `start-end` immutable as the smoke-test template. The typed `request-approval` builder now matches the Human-example six-node topology: a primary SmartForms item reference, pending/approved/rejected request updates, three Originator emails, Originator Manager task, Approved/Rejected decision routing, native Start/Task form rules, and explicit absolute connector geometry. Add rework loops next, followed by reminders/escalations and subworkflows.
