...
- `SaveKprx(close=true)` does not reliably clear the HTML5 designer lock. Invoke `UnlockProcess` with `K2:<DOMAIN>\<user>` and confirm `GetUserProcessKprx.LockedBy` is empty.
...
Keep `start-end` immutable as the smoke-test template. The typed `request-approval` builder now covers a primary SmartForms item reference, request status Update, Originator email, Originator Manager task, and native Start/Task form rules. Add decision routing and outcome-specific status updates next, followed by reminders/escalations and subworkflows.
