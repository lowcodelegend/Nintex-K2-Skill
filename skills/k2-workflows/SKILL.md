...
5. Run `inspect` and `verify`. Verification must prove both the saved JSON and, when `publish=true`, the runtime process definition. A direct human task must be assigned exclusively to Originator for the testing/demo baseline; an approval-matrix task must be assigned to the resolver's approver data field and must prove its resolver/decision/loop topology. For SmartForms integration it must also prove that the declared Start and Task states exist, the Task state is not default, and the Start state's runtime default matches `makeStartStateDefault`.
