...
Select the data backend before creating specialist manifests. Ask whether to use native SmartBox or SQL Server whenever the request says only SmartObject/data model. Use SmartBox for simple K2-owned CRUD and SQL for relational integrity, procedures, reporting, approval matrices, integration, or material scale.

Use the dependency direction `data storage/SmartObjects in <root>\Data → views → forms → workflow → form integration rules`. SQL adds database objects and a Service Instance before its generated SmartObjects; SmartBox uses the installed native Service Instance. Deploy and verify in that order. Cleanup, when authorized, runs in reverse order.
...
- SmartForm view bindings must name SmartObjects declared by the selected data specialist.
...
- Every specialist `application.rootCategoryPath` must equal the solution root. Both SmartObject CLIs derive the fixed `<root>\Data` child from that value; do not configure a separate SmartObject root.
