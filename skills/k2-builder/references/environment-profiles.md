...
On this environment, the user-approved PSF contract is: Style Profile `PSF UX v1`; header `PSF.FrameworkHeader` with instance name `Header`, blank visible title, and `IsCollapsible=false`; footer `PSF.FrameworkFooter` in the last view position; header `Init` called from Form initialization with `AppId={{solution.code}}` and `Debug=false`; then Form server load calls header `ServerPreRender` before one transfer-data action sets `{{form.name}}` on `Main Header Data Label` and `{{application.name}}` on `Sub Header Data Label`. Do not pass title/subtitle through the header's `HeaderText` and `SubheaderText` parameters. This paragraph records a discovered environment example, not a portable assumption.
