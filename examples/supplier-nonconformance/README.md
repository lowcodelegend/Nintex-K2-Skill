...
- `SNC.New Nonconformance` guided initiation with a native Progress stepper, explicit current-screen heading, current-screen Continue validation, non-destructive Back navigation, Case Details, Evidence, saved-key Review & Submit, a dedicated final Submit case action, and native start-only integration to the parent lifecycle. The mapping intentionally consolidates the five conceptual Northstar questions into three persistence-safe K2 screens.
